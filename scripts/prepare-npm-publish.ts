...
export function prepareNpmPublish(options: PrepareOptions = {}): void {
  ...
  const publishName = validatePackageName(currentName)
  ...
  console.log(`NPM publish name: ${publishName}`)
...
if (isDirectRun) {
  prepareNpmPublish({ version: process.env.PUBLISH_VERSION })
}
